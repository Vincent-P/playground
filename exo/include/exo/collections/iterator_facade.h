...
struct arrow_proxy
{
    ...
    explicit constexpr arrow_proxy(Reference &&r) noexcept
        : _value(std::forward<Reference>(r))
    {
    }
...
{
template <typename Iterator>
requires std::is_base_of_v<exo::IteratorFacade<Iterator>, Iterator> struct iterator_traits<Iterator>
{
    static const Iterator &_it;

    using difference_type   = exo::infer_difference_type_t<Iterator>;
