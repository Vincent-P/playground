...
layout(set = 1, binding = 2) uniform sampler2D MultiScatTexture; // sampler linear clamp
...
void main()
{
    ...
    float2 uv = (pixPos) / float2(global.TRANSMITTANCE_TEXTURE_WIDTH, global.TRANSMITTANCE_TEXTURE_HEIGHT);
    float viewHeight;
    ...
    float3 transmittance = float3(exp(-IntegrateScatteredLuminance(pixPos, WorldPos, WorldDir, global.sun_direction, Atmosphere, ground, SampleCountIni, DepthBufferValue, VariableSampleCount, MieRayPhase, 9000000.0f).OpticalDepth));
