...
void main()
{
	...
	vec3 v = normalize(ubo.cam_pos.xyz - inWorldPos);    // Vector from surface point to camera
	vec3 l = normalize(ubo.light_dir.xyz);     // Vector from surface point to light
	...
	if (ubo.debugViewInput > 0.0) {
		int index = int(ubo.debugViewInput);
		switch (index) {
			case 1:
