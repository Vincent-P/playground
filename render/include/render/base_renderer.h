...
struct BaseRenderer
{
	static BaseRenderer *create(exo::ScopeStack &scope, cross::Window *window, gfx::DeviceDescription desc);
	~BaseRenderer();
	...
	template <typename T> T *bind_compute_shader_options(gfx::ComputeWork &cmd)
	{
		return (T *)bind_compute_shader_options(cmd, sizeof(T));
	}
	template <typename T> T *bind_graphics_shader_options(gfx::GraphicsWork &cmd)
	{
		return (T *)bind_graphics_shader_options(cmd, sizeof(T));
	}

	void                    *bind_global_options(gfx::GraphicsWork &cmd, usize options_len);
	template <typename T> T *bind_global_options(gfx::GraphicsWork &cmd)
	{
		return (T *)bind_global_options(cmd, sizeof(T));
	}
