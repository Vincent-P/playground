...
static usize temp_bvh_split_sah(Vec<TempBVHNode> &temp_nodes, TempBVHNode &node, usize prim_start, usize prim_end)
{
	...
	std::sort(/*std::execution::par_unseq, */
	          ...
	          [&](const TempBVHNode &a, const TempBVHNode &b) {
				  return a.bbox_center[i_max_comp] < b.bbox_center[i_max_comp];
			  });
	...
	for (usize i_split_bucket = 0; i_split_bucket < BUCKET_COUNT - 1; i_split_bucket += 1) {
		...
		if (left_count > 0 && right_count > 0) {
			...
			cost =
				RAY_BOX_COST +
				(static_cast<float>(left_count) * left_area + static_cast<float>(right_count) * right_area) / node_area;
		}
...
static void create_nodes(BVHScratchMemory &scratch, Vec<BVHNode> &nodes)
{
	...
	for (usize i_temp_node = 0; i_temp_node < temp_nodes.size(); i_temp_node += 1) {
		...
		node.next_node  = temp_node.next_node_index == u64_invalid
		                      ? u32_invalid
		                      : static_cast<u32>(temp_nodes[temp_node.next_node_index].depth_first_index);
                 ...
                 const Vec<u32>      &blas_indices)
{
	...
	for (usize i_blas = 0; i_blas < blas_roots.size(); i_blas += 1) {
		...
		for (usize i_corner = 0; i_corner < exo::Array::len(corners); i_corner += 1) {
			corners[i_corner] = (transform * float4(corners[i_corner], 1.0)).xyz();
		}

		node.bbox.min = corners[0];
		node.bbox.max = corners[0];
		for (usize i_corner = 1; i_corner < exo::Array::len(corners); i_corner += 1) {
