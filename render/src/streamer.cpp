...
static bool copy_to_staging(Streamer &streamer, const void *data, usize len)
{
	...
	if (previous_update_start && streamer.cursor < previous_update_start &&
	    streamer.cursor + len > previous_update_start) {
		return false;
	}
...
void Streamer::destroy() { device->destroy_buffer(cpu_buffer); }
...
bool Streamer::upload_image_regions(Handle<gfx::Image>           image,
                                    const void                  *data,
                                    usize                        len,
...
bool Streamer::upload_buffer_regions(Handle<gfx::Buffer>           buffer,
                                     const void                   *data,
                                     usize                         len,
