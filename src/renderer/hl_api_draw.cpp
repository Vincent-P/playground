...
static RenderPass &find_or_create_render_pass(API &api, PassInfo &&info)
{
    ...
    vk::AttachmentReference depth_ref(0, separateDepthStencilLayouts ? vk::ImageLayout::eDepthAttachmentOptimal
...
static void bind_descriptor_set(API &api, Program &program, uint i_set)
{
    ...
    cmd.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, *program.pipeline_layout, i_set, descriptor_set.set,
			   offsets);
}
...
void API::bind_image(ProgramH program_h, uint set, uint slot, ImageH image_h)
{
    ...
    assert(image.layout == vk::ImageLayout::eShaderReadOnlyOptimal
	   || image.layout == vk::ImageLayout::eDepthStencilReadOnlyOptimal);
...
static void pre_draw(API &api, Program &program) { bind_descriptor_set(api, program, DRAW_DESCRIPTOR_SET); }
