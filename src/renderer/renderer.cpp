...
static void bind_texture(Renderer &r, uint slot, std::optional<u32> i_texture)
{
	...
	r.api.bind_combined_image_sampler(r.model.program, vulkan::DRAW_DESCRIPTOR_SET, slot, image.image_h,
					  sampler.sampler_h);
    }
    else {
	// bind empty texture
...
static void draw_node(Renderer &r, Node &node)
{
    ...
    *buffer      = node.cached_transform;
	...
	r.api.draw_indexed(primitive.index_count, 1, primitive.first_index, static_cast<i32>(primitive.first_vertex),
			   0);
    }
...
static void shadow_map(Renderer &r)
{
	...
	static float3 s_sun_angles = {80.0f, 0.0f, 0.0f};
