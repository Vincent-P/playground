...
namespace my_app
{
    ...
    {
        std::string err, warn;
        if (!loader.LoadASCIIFromFile(&model_, &err, &warn, path) || !err.empty() || !warn.empty())
        {
            if (!err.empty())
                std::clog << err;
            if (!warn.empty())
        ...
        for (const auto& mesh : model_.meshes)
        {
            ...
            for (const auto& primitive : mesh.primitives)
            {
...

                {
                    ...
                    auto vertices = reinterpret_cast<float*>(
                        &(model_.buffers[position_view.buffer].data[total_offset]));
                    ...
                    switch (indices_acc.componentType)
                    {
                        case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT:
                        {
                            auto indices = reinterpret_cast<uint16_t*>(
                                &(model_.buffers[indices_view.buffer].data[total_offset]));
