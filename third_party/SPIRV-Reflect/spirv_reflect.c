...
)
{
  ...
  switch (op_type) {
    ...
    case SpvOpTypeStruct: {
      ...
      // tmp hack: https://github.com/KhronosGroup/SPIRV-Reflect/issues/78#issuecomment-545123389
  ...
  for (size_t i = 0; i < io_var_count; ++i) {
    ...
    if (p_node->storage_class == SpvStorageClassInput) {
     ...
     p_var->storage_class = SpvStorageClassInput;
      ...
      // tmp hack
      // Starting with version 1.4, the interface’s storage classes are all storage classes
