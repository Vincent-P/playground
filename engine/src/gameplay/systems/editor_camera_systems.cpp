...
void EditorCameraTransformSystem::update(const UpdateContext &ctx)
{
    ...
    case EditorCameraState::Idle:
    {
        ...
        {
            editor_camera_component->target.y += (CAMERA_SCROLL_SPEED * static_cast<float>(ctx.delta_t)) * static_cast<float>(camera_scroll.y);
        }
    ...
    case EditorCameraState::Move:
    {
        ...
        else
        {
            ...
            if (mouse_delta)
            {
                ...
                editor_camera_component->target = editor_camera_component->target + CAMERA_MOVE_SPEED * static_cast<float>(ctx.delta_t) * mouse_up * camera_plane_forward;
            }
        }
        break;
    }
    ...
    case EditorCameraState::Zoom:
    {
        ...
        {
            editor_camera_component->r += (CAMERA_SCROLL_SPEED * static_cast<float>(ctx.delta_t)) * static_cast<float>(camera_scroll.y);
    ...
    ImGui::Text("[%s] Position: %f, %f, %f", to_string(ctx.stage), position.x, position.y, position.z);
}
