...
VkDescriptorSetLayout dynamic_buffer_layout = VK_NULL_HANDLE;
...
void create_dynamic_buffer_layout(Device &device)
{
	...
	VK_CHECK(vkCreateDescriptorSetLayout(device.device, &desc_layout_info, nullptr, &dynamic_buffer_layout));
}
...
DynamicBufferDescriptor create_buffer_descriptor(Device &device, Handle<Buffer> &buffer_handle, usize range_size)
{
	...
	if (dynamic_buffer_layout == VK_NULL_HANDLE)
	{
	...
	set_info.pSetLayouts                 = &dynamic_buffer_layout;
	...
	vkUpdateDescriptorSets(device.device, ARRAY_SIZE(writes), writes, 0, nullptr);
...
{
	vkDestroyDescriptorSetLayout(device.device, dynamic_buffer_layout, nullptr);
	dynamic_buffer_layout = VK_NULL_HANDLE;
}
...
BindlessSet create_bindless_set(const Device &device, u32 sampler_count, u32 image_count, u32 buffer_count)
{
	...
	VkDescriptorBindingFlags flags[ARRAY_SIZE(bindings)] = {};
	for (usize i = 0; i < ARRAY_SIZE(bindings); i += 1) {
    ...
    bindless.free_list[BindlessSet::PER_SAMPLER] = exo::FreeList::create(sampler_count);
    bindless.free_list[BindlessSet::PER_IMAGE] = exo::FreeList::create(image_count);
    bindless.free_list[BindlessSet::PER_BUFFER] = exo::FreeList::create(buffer_count);
...
void destroy_bindless_set(const Device &device, BindlessSet &bindless)
{
    ...
    bindless.free_list[BindlessSet::PER_BUFFER].destroy();
}

u32 bind_sampler_image(BindlessSet &bindless, Handle<Image> image_handle)
{
    u32 new_index = bindless.free_list[BindlessSet::PER_SAMPLER].allocate();
...
{
    u32 new_index = bindless.free_list[BindlessSet::PER_IMAGE].allocate();
...
{
    u32 new_index = bindless.free_list[BindlessSet::PER_BUFFER].allocate();
...
void unbind_sampler_image(BindlessSet &set, u32 index)
{
    ...
    set.free_list[BindlessSet::PER_SAMPLER].free(index);
...
void unbind_storage_image(BindlessSet &set, u32 index)
{
    ...
    set.free_list[BindlessSet::PER_IMAGE].free(index);
...
void unbind_storage_buffer(BindlessSet &set, u32 index)
{
    ...
    set.free_list[BindlessSet::PER_BUFFER].free(index);
...
void update_bindless_set(Device &device, BindlessSet &bindless)
{
	...
	usize total_bind_count = bindless.pending_bind[BindlessSet::PER_SAMPLER].size() + bindless.pending_bind[BindlessSet::PER_IMAGE].size() + bindless.pending_bind[BindlessSet::PER_BUFFER].size();
	usize total_unbind_count = bindless.pending_unbind[BindlessSet::PER_SAMPLER].size() + bindless.pending_unbind[BindlessSet::PER_IMAGE].size() + bindless.pending_unbind[BindlessSet::PER_BUFFER].size();

	auto *descriptor_writes = exo::tls_allocator.allocate<VkWriteDescriptorSet>(total_bind_count);
	auto *descriptor_copies = exo::tls_allocator.allocate<VkCopyDescriptorSet>(total_unbind_count);
	...
	usize total_image_info_count = bindless.pending_bind[BindlessSet::PER_SAMPLER].size() + bindless.pending_bind[BindlessSet::PER_IMAGE].size();
	usize total_buffer_info_count = bindless.pending_bind[BindlessSet::PER_BUFFER].size();

	VkDescriptorImageInfo  *image_infos  = total_image_info_count > 0 ? exo::tls_allocator.allocate<VkDescriptorImageInfo>(total_image_info_count) : nullptr;
	VkDescriptorBufferInfo *buffer_infos  = total_buffer_info_count > 0 ? exo::tls_allocator.allocate<VkDescriptorBufferInfo>(total_buffer_info_count) : nullptr;
	...
	for (u32 i_set = 0; i_set < bindless.pending_bind.size(); i_set += 1) {
...
{
	return set.sampler_images[index];
}

Handle<Image> get_storage_image_at(BindlessSet &set, u32 index)
{
	return set.storage_images[index];
}
