		    ...
		    std::span<std::pair<Handle<Buffer>, BufferUsage>> buffers)
{
	...
	for (auto &[image_handle, usage_dst] : images) {
		...
		image_barriers.push_back(
			get_image_barrier(image.vkhandle, src_access, dst_access, image.full_view.range));
	...
	for (auto &[buffer_handle, usage_dst] : buffers) {
		...
		buffer_barriers.push_back(
			get_buffer_barrier(buffer.vkhandle, src_access, dst_access, 0, buffer.desc.size));
		src_stage |= src_access.stage;
		dst_stage |= dst_access.stage;
...
void Device::reset_work_pool(WorkPool &work_pool)
{
	...
	for (auto &command_pool : work_pool.command_pools) {
		...
		VK_CHECK(vkResetCommandPool(
			this->device, command_pool.vk_handle, VK_COMMAND_POOL_RESET_RELEASE_RESOURCES_BIT));
	}
}
